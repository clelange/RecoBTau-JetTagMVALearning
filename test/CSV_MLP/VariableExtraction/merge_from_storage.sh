...
maindirec=/pnfs/psi.ch/cms/trivcat/store/user/clange/CSVtraining_allVars/
...
homedirec=/scratch/clange/CSVtraining_allVars
...
do
	for k in ${CAT[@]} ;
	do
		for j in $( ls $maindirec/$l/${k}_B*); do printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}Bfiles.txt; done
		for j in $( ls $maindirec/$l/${k}_C*); do printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}Cfiles.txt; done
		...
		then
			...
			for j in $( ls $maindirec/$l/${k}_D*); do
				...
				printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}DUSGfiles.txt
			done
			let countfiles=0
			...
			for j in $( ls $maindirec/$l/${k}_D*); do printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}DUSGfiles.txt; done
			for j in $( ls $maindirec/$l/${k}_U*); do printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}DUSGfiles.txt; done
			for j in $( ls $maindirec/$l/${k}_S*); do printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}DUSGfiles.txt; done
			for j in $( ls $maindirec/$l/${k}_G*); do printf "dcap://t3se01.psi.ch:22125/$j " >> ${k}DUSGfiles.txt; done			
		fi
	done
done	
